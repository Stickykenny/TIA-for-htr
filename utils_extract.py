...
def extract_file_from_drive( csv_source : str, c1 : int,c2 : int=-1, list_to_compare : list[str]= []) :
    """
    ...
    with open(csv_source, newline='') as inputfile:
        if c1 >= 0 and c2 >= 0 :
            csv_data = sorted([(row[c1],row[c2]) for row in csv.reader(inputfile) if row[c2] != ""],key=lambda x : x[1])
            i = j = 0
...
if __name__ == "__main__" :
    ...
    print("extract")
    print(extract_file_from_drive(csv_source,c1=3,c2=9,list_to_compare=list(letters_fetched.keys())))